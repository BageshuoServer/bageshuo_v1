...
public class CommunityService {
	...
	public List<UserCommunityInfo> getUserCommunityInfo(int u_id,
		...
		try {
			boolean isExits = userdao
					.queryUserByUidAndDeviceId(u_id, device_id);
			if (isExits) {
				// 2根据用户u_id获得co_id列表
				...
				if (!communityUserList.isEmpty()) {
					...
					for (CommunityUser communityUser : communityUserList) {
						...
						community = communityDao
								.queryCommunityByCoId(communityUser.getCo_id());
						// 3根据co_id获得社区中的帖子数
						invitationList = invitationDao
								.queryInvatationListByCoId(communityUser
										.getCo_id());
						// 封装每一个信息
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> perCommunityInfo(int u_id, int co_id,
			String device_id) throws Exception {
		...
		try {
			...
			if (isExits) {
				...
				// 将得到的信息放到map中
				perCommunityInfoMap
						.put("co_interest_num", communityList.size());
	 ...
	 * @throws Exception
	 */
	public List<HashMap<String, Object>> getTopInvationInfoList(int u_id,
			String device_id, int co_id) throws Exception {
		...
		try {
			...
			if (isExits) {
				...
				topInvationList = invitationDao
						.queryTopInvationListByCoIdOrderByInvTime(co_id);
				if (!topInvationList.isEmpty()) {
					...
					for (Invitation invitation : topInvationList) {
						topInvitationNum = commentDao
								.queryCommentNumListByInvId(invitation
										.getInv_id());
	 ...
	 * @throws Exception
	 */
	public boolean interestCo(int u_id, int co_id, String device_id)
			throws Exception {
		boolean interestSuccess = false;
		try {
			boolean isExsit = userdao
					.queryUserByUidAndDeviceId(u_id, device_id);
			if (isExsit) {
				// 判断用户是否已经关注这个社区
				boolean isAleadlyInterest = communityUserDao
						.queryUserIsInerest(u_id, co_id);
				if (isAleadlyInterest) {
					// 如果关注则赋值关注成功
					interestSuccess = true;
				} else {
					// 用户还没关注，则向中间表中插入数据
					interestSuccess = communityUserDao.insertIntoComminityUser(
							u_id, co_id);
	 ...
	 * @throws Exception
	 */
	public boolean cancelInterestCo(int u_id, int co_id, String device_id)
			throws Exception {
		boolean cancelInterestSuccess = false;
		try {
			boolean isExsit = userdao
					.queryUserByUidAndDeviceId(u_id, device_id);
			if (isExsit) {
				...
				if (isAleadlyInterest) {
					// 如果关注则赋值关注成功
					cancelInterestSuccess = communityUserDao
							.deleteCommunityUserByUidAndCoid(co_id, u_id);
					return cancelInterestSuccess;
				} else {
					// 用户还没关注，则向中间表中插入数据
					cancelInterestSuccess = true;
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param inv_voice
	 * @param inv_share_num
	 * @param inv_praise_num
	 * @param inv_top
	 * @param inv_outstanding
	 * @param u_id
	 * @param co_id
	 * @return
	 * @throws Exception
	 */
	public boolean publishInv(String inv_location, java.util.Date inv_time,
			String inv_word, String inv_voice, int inv_share_num,
			int inv_praise_num, int inv_top, int inv_outstanding, int u_id,
			int co_id) throws Exception {
		boolean flag = false;
		// 将数据保存到数据库服务器
		flag = invitationDao.insertInvitation(inv_location, inv_time, inv_word,
	...
	public boolean publishComment(String com_location,
		...
		boolean ispublishComment = false;
		try {
			ispublishComment = commentDao.insertCommentByInvId(com_location, com_time, com_word, com_voice, u_id, inv_id);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
