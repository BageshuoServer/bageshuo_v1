...
public class UserService {
	 ...
	 * @param user
	 *            产生的用户
	 * @return -1用户已存在， -2八哥号创建失败 -3环信创建失败 大于0成功返回u_id
	 */
	public int userRegister(String u_name, String u_pwd, String u_type,
		...
		if (isExist) {// 用户已存在
			...
			u_id = userDao.saveUser(getUser(u_name, u_pwd, u_type, device_id,
					u_push_id));
			isSuccess = u_id;
	...
	public int updatePwd(String u_name, String u_pwd, String device_id)
		...
		if (isExist) {
			...
			if (code == 200) {
				// 修改服务器密码
				boolean isUpdate = userDao.updatePwdAndDeviceID(u_pwd,
						device_id, u_name);
				if (isUpdate) {
	...
	public boolean setUserInfo(String u_head, String device_id, String u_nick,
		...
		boolean flag = false;
		// 将数据保存到数据库服务器
		flag = userDao.updateUserInfo(device_id, u_head, u_nick, u_sex, u_home,
				u_sign, u_id);
		return flag;
	}
	 ...
	 * @param u_push_id
	 *            clientid
	 * @return -2八哥号创建失败 -3环信创建失败 大于0成功返回u_id
	 */
	public int threeLogin(String u_name, String u_pwd, String u_type,
		...
		boolean isExist = userDao.queryUserByUName(u_name);
		if (isExist) {// 存在，更新数据
			boolean isUpdate = userDao.updateTypeDeviceIDPushID(u_type,
					device_id, u_push_id, u_name);
			if (isUpdate) {
				uId = userDao.queryUidByUname(u_name);
				isSuccess = uId;
			...
			uId = userDao.saveUser(getUser(u_name, u_pwd, u_type, device_id,
					u_push_id));
			isSuccess = uId;
	...
	private boolean buildBage(int u_id) throws SQLException {
		...
		System.out.println("八哥号：" + first + "-" + second + "-" + third + "-"
				+ u_id);
	 ...
	 * @throws Exception
	 */
	public User getUserInfoByUid(int target_u_id, int u_id, String device_id)
			throws Exception {
		User user = null;
		...
		if (isExist) {
			try {
				user = userDao.queryUserByUid(target_u_id);
			} catch (Exception e) {
				e.printStackTrace();
				throw e;
			}
