package com.feytuo.bageshuo.util.huanxin;
...
//		loginHX("111", "111");
	 ...
	 * 注册IM用户[单个]
	 */
	public static int registerHX(String userName, String pwd) {
		int code = 0;
...
//					+ createNewIMUserSingleNode.toString());
			code = createNewIMUserSingleNode.get("statusCode").asInt();
		}
		return code;
	}
	 ...
	 * IM用户登录
	 */
	public static int loginHX(String userName, String pwd) {
		int code = 0;
		ObjectNode imUserLoginNode = imUserLogin(userName, pwd);
		if (null != imUserLoginNode) {
//			LOGGER.info("IM用户登录: " + imUserLoginNode.toString());
//			System.out.println("IM用户登录: " + imUserLoginNode.toString());
			code = imUserLoginNode.get("statusCode").asInt();
		}
		return code;
	}
	 ...
	 * 重置IM用户密码 提供管理员token
	 */
	public static int modifyPwdHX(String userName, String newPwd) {
		int code = 0;
		...
		if (null != modifyIMUserPasswordWithAdminTokenNode) {
			code = modifyIMUserPasswordWithAdminTokenNode.get("statusCode").asInt();
//			System.out.println("重置IM用户密码 提供管理员token: "
//					+ modifyIMUserPasswordWithAdminTokenNode.toString());
		}
		ObjectNode imUserLoginNode = imUserLogin(userName,
				json.get("newpassword").asText());
		if (null != imUserLoginNode) {
//			LOGGER.info("重置IM用户密码后,IM用户登录: " + imUserLoginNode.toString());
//			System.out.println("重置IM用户密码后,IM用户登录: "
//					+ imUserLoginNode.toString());
		}
		return code;
	}
